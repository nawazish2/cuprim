...
/// Cuprim requires Apple Silicon + macOS 26+ for true Liquid Glass.
enum PlatformRequirements {
    ...
    private static func presentUnsupportedAlert() {
        let alert = NSAlert()
        alert.messageText = "Cuprim can’t run on this Mac"
        if !isAppleSilicon {
            alert.informativeText = "Cuprim requires an Apple Silicon Mac (M1 or later)."
        } else {
            alert.informativeText = "Cuprim requires macOS 26 or later for Liquid Glass."
        }
