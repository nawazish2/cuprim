...
enum AppUpdater {
    ...
    static func checkForUpdates(interactive: Bool = true) {
        ...
        Task {
            ...
            do {
                ...
                if compareVersions(remote, current) <= 0 {
                    if interactive {
                        present(
                            ...
                            message: "TokenBar \(current) is the current version."
                        )
                    }
                    return
                }
                ...
                showProgress("Downloading TokenBar \(remote)…")
    ...
    private static func fetchLatestRelease() async throws -> ReleaseDTO {
        let url = URL(string: "https://api.github.com/repos/nawazish2/tokenbar/releases/latest")!
    ...
    private static func download(asset: AssetDTO) async throws -> URL {
        ...
        let dest = FileManager.default.temporaryDirectory
            .appendingPathComponent("TokenBar-update-\(UUID().uuidString).zip")
    ...
    private static func installAndRelaunch(zipURL: URL) throws {
        ...
        let workDir = fm.temporaryDirectory
            .appendingPathComponent("TokenBar-update-\(UUID().uuidString)", isDirectory: true)
        ...
        while /usr/bin/pgrep -x TokenBar >/dev/null 2>&1; do
          sleep 0.25
        done
        /bin/rm -rf "$TARGET"
    ...
    private static func findAppBundle(in directory: URL) -> URL? {
        ...
        if let enumerator = fm.enumerator(
            ...
            for case let url as URL in enumerator {
                ...
                if url.lastPathComponent == "TokenBar.app" {
                    return url
                }
                if fallback == nil {
                    fallback = url
    ...
    private static func confirmUpdate(from current: String, to remote: String, notes: String?) -> Bool {
        ...
        var text = "TokenBar \(remote) is available (you have \(current)).\n\nDownload and install now?"
    ...
    enum UpdateError: LocalizedError {
        ...
        var errorDescription: String? {
            ...
            case .appMissing:
                return "The download didn’t contain TokenBar.app."
            }
        }
    }
}
