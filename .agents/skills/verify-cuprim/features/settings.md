...
- **Open (manual).** Status menu → **Settings…** (or activate app and press ⌘,). Native grouped Form: Providers / Display / General / About.
- **Screenshot.** Capture the window → `evidence/<slug>/ui-settings.png`.
- **Provider order (manual, if claimed).** Drag rows to reorder; reopen status menu and confirm order matches.
