...
About TokenBar shows the branded about panel: icon, name, version, author credit, and GitHub link.
...
- Status menu → **About TokenBar**
- App menu → **About TokenBar**
- Settings → About → **About TokenBar…**
...
- **Open (manual).** Choose **About TokenBar**. Centered about UI with TokenBar title.
- **Version check.** Compare on-screen version to:

  ```bash
  /usr/libexec/PlistBuddy -c 'Print :CFBundleShortVersionString' dist/TokenBar.app/Contents/Info.plist
  ```
