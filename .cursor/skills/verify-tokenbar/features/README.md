# TokenBar verification map

Maintained source for verifying user-facing TokenBar behavior. Read this index, then the feature file that matches the claim.

## Baseline preconditions

- Host is **Apple Silicon** (`arm64`) and **macOS 26+**
- Work from the repo root; prefer `dist/TokenBar.app` built by this run
- `swift test` passes for TokenBarCore claims; package with `CONFIG=debug LAUNCH=0 ./script/package_app.sh` when you need a runnable `.app`
- Launch with `./script/build_and_run.sh` only when a live menu-bar instance is required
- Do not drive a TokenBar you did not start for this verification run when the claim depends on your binary
...
- UI proof = screenshot of the status menu / panel / window showing TokenBar chrome + the claimed state
- Build proof = log + exit code `0` + `dist/TokenBar.app` exists
...
Each feature file: H1 + short description, then exactly four H2s — `Sub-features`, `How to get to it (user POV)`, `Driving it with verify-tokenbar`, `Gotchas`.

## Features

- [Menu-bar status](./menu-bar-status.md) — gauge, usage menu, refresh, quit
- [Dashboard panel](./dashboard-panel.md) — glass “Show TokenBar” panel
