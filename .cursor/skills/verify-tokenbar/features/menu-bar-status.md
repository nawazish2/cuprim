...
- `status-visible` shows the TokenBar status item in the menu bar after launch
...
- `quit` exits via **Quit TokenBar** (⌘Q)

## How to get to it (user POV)

- Launch TokenBar (`./script/build_and_run.sh` or open `dist/TokenBar.app`)
- Click the gauge / chart status item
- Choose **Refresh**, a provider row, or **Quit TokenBar**

## Driving it with verify-tokenbar

Preconditions:

- Doctor passed on an arm64 macOS 26+ host
- This run’s `dist/TokenBar.app` is running (or just packaged for build-only claims)
...
- **Quit (manual).** Choose **Quit TokenBar**. `pgrep` no longer lists this run’s binary.
...
- Do not confuse `/Applications/TokenBar.app` with this checkout’s `dist/` build
